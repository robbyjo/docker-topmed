...
RUN cd /home && wget -q ftp://ftp.csx.cam.ac.uk/pub/software/programming/pcre/pcre-8.40.tar.gz && \
...
RUN ln -sf /opt/intel/lib/intel64/libiomp*.so /usr/lib && cd /home && \
...
RUN cd /home/R-3.4.0 && \
  ...
  ./configure CFLAGS="-g -O3" CPPFLAGS="-g -O3" FFLAGS="-g -O3" FCFLAGS="-g -O3 -m64 -I${MKLROOT}/mkl/include" --prefix=/opt/R --enable-shared --enable-R-profiling --enable-memory-profiling --disable-java --with-blas="$MKL" --with-lapack

RUN cd /home/R-3.4.0 && make

#RUN cd /home/R-3.4.0 && make check

RUN cd /home/R-3.4.0 && make install && cd /home && rm -Rf /home/R-3.4.0*
